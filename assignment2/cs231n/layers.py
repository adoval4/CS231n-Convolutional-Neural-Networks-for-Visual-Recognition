...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # itearate over input sample (total = N)
    for n in range(N):
        n_sample = x_pad[n]
        # iterate over each filter and bias (total = F)
        for f in range(F): 
            f_filter = w[f, :]
            f_bias = b[f]
            # iterate over each convolution (each pixel of the output)
            for _h in range(_H):
                for _w in range(_W):
                    # extract the piece of the sample where we will apply the convolution filter
                    sample_patch = n_sample[:, _h * stride:_h * stride + HH, _w * stride:_w * stride + WW]
                    # apply filter    
                    convolution = np.sum(sample_patch * f_filter)
                    # save the resulting convolution into the rigth pixel (_h, _w) of the output including bias
                    out[n, f, _h, _w] = convolution + f_bias
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        ...
        for c in range(C):
            for j in range(_H):
                for i in range(_W):
                    # get sample path out of sample 
                    sample_patch = n_sample[c, j * stride: j * stride + _H, i * stride : i * stride + _W ]
                    # apply max_pool
                    out[n, c, j, i] = np.max(sample_patch)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        ...
        for c in range(C):
            for j in range(_H):
                for i in range(_W):
                    # get patch from sample
                    patch = n_sample[c, j * stride: j * stride + _H, i * stride : i * stride + _W ]
                    # get the mask of the max
                    pooling_mask = patch == np.max(patch)
                    # apply gradient with mask
                    dx[n, c, j * stride: j * stride + _H, i * stride : i * stride + _W] += dout[n, c, j, i] * pooling_mask 
